...
use App\Notifications\UserEnrollmentNotification;
use Carbon\Carbon;
...
class UserController extends Controller
{
    public function List()
    {
        ...
        return view('backend.pages.user_list_page', compact('data', 'users'));
    }

    public function enrollmentDate($id)
    {
        $enrollments = UserEnrollment::where('user_id', $id)->get();

        return view('backend.pages.enrollment_date', compact('enrollments'));
    }
    ...
    //             ]);
    //             $data->notify(new UserEnrollmentNotification($request));
    //         });
    ...
    public function deleteEnrollmentCourse($id)
    {
        $enrollment = UserEnrollment::find($id);
        $enrollment->delete();
        return back()->with('user_deleted', 'User record has been deleted successfully!');
    }
