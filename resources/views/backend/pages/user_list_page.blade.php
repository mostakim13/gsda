            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="#" class="btn btn-success" data-toggle="modal" data-target="#"><i
                                        ...
                                        <td>
                                            @if (count($row->user_enrollments) > 0)
                                                @foreach ($row->user_enrollments as $enroll)
                                                    <span
                                                        class="badge bg-warning text-dark">{{ $enroll->course->course_title }}</span>
                                                @endforeach
                                            @endif
                                        </td>
                                        <td>
                ...
                $.ajax({
                    ...
                    error: function(error_response) {
                        ...
                        $.each(error_response.responseJSON, function(key, value) {
                            ...
                            $('#error_span').append('<li>' + value + '</li>').addClass(
                                'alert alert-danger');
                            $('#' + key).parent().parent().append(
                                '<div class="col-sm-7 help-block">' + value + '</div>').closest(
                                '.form-group').removeClass('has-error').addClass('has-error');
